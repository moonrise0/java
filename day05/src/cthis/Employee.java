...
public class Employee {
	private String id;
	private String name; // 소스에서 컨스터럭터 유징 필드.
	private int salary;   //프라이빗해서 값을 가져오기가 힘듬.

	public Employee() { // 디폴트값 설정.
		Calendar cal = Calendar.getInstance(); //컨트롤 시프트 o누르면 자동임포트
		this.id = "" + cal.get(Calendar.YEAR) + cal.get(Calendar.MILLISECOND);  //""은 string으로 값을 바꿔줌
	}
	
	 public Employee(String name, int salary) { //id는 자동생성함으로 안받는다
			this(); 
		    this.name = name;
			this.salary = salary;
		}
